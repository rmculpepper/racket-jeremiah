...
@;{
This is a Scribble block comment.

Some differences from Frog:

- Jeremiah reads metadata from Racket-style line comments at the
  beginning of the file, before the #lang line.

- If no Title is declared in the header, Jeremiah uses the title set
  using Scribble's @title form, if one is present. (But if both are
  present the header takes precedence.)

}

@; This is a Scribble line comment.
